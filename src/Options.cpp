    ...
    {   
        ...
        ("config,c", po::value<vector<string>>()->composing(),"Config file, multiple allowed")
    ...
    {
        od.add_options()
        ("psql",po::value<string>()->required(), "PostgreSQL connection string")
        ("perf_detail",po::value<bool>()->default_value(false), "Include performance details in logs")
        ("kline.cycle_time",po::value<int>()->default_value(1), "Binance REST AIP endpoint (implicit port is https)")
        ("binance.api",po::value<string>()->required(), "Binance REST AIP endpoint (implicit port is https)")
        ("binance.wss",po::value<string>()->required(), "Binance WebSocket endpoint address")
        ("binance.wssport",po::value<string>()->required(), "Binance WebSocket endpoint port");
    }
    ...
    BOOST_LOG_ATTRIBUTE_KEYWORD(_threadid, "ThreadID",  logging::attributes::current_thread_id::value_type)
    ...
    void Options::setup_logging_console() {
        ...
        //const int console_lvl = 99999; //angry debugin!
        
        logging::add_console_log(clog, 
                                logging::keywords::format = //"%TimeStamp%: [%Severity%] %_%",
